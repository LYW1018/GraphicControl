...
@section('Content')
    ...
    <div class="box mb-3">
        @foreach($vWord as $value)
            @if($value['Name'] === 'D2000')
                <label>{{$value['Name']}}({{$value['Address']}}) Write</label>
                <input id="{{$value['Name']}}_Word_Write" type="button"
                       data-id="{{$value['Name']}}" data-guid="{{$value['Guid']}}" data-name="{{$value['Name']}}_Word_Write"
                       class="button form-control" value="{{$value['NowValue']}}">
            @endif
        @endforeach
        @foreach($vString as $value)
            @if($value['Name'] === 'D2006')
                <label>{{$value['Name']}}({{$value['Address']}}) Write String</label>
                <input id="{{$value['Name']}}_String_Write" type="button"
                       data-id="{{$value['Name']}}" data-guid="{{$value['Guid']}}" data-name="{{$value['Name']}}_String_Write"
                       class="button form-control" value="{{$value['NowValue']}}">
            @endif
        @endforeach
    </div>
    ...
    <div class="box mb-3 w-100 align-middle">
        <div class="col text-center">
            <button id="Y?" class="btn btn_switch3">Y?</button>
        </div>
        <div class="col text-center">
            <button id="Y?" class="btn btn_switch4">Y?</button>
        </div>
        <div class="col text-center">
            <button id="Y?" class="btn btn_switch5">Y?</button>
        </div>
    </div>
@stop
